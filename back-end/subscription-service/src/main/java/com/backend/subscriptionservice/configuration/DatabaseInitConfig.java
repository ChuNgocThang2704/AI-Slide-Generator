...
public class DatabaseInitConfig implements BeanFactoryPostProcessor, EnvironmentAware {
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        ...
        log.info("[subscription-service] Checking if PostgreSQL database exists for URL: {}", url);
        try {
            String cleanUrl = url;
            ...
            try (Connection conn = DriverManager.getConnection(baseUri, username, password)) {
                try (Statement stmt = conn.createStatement()) {
                    ...
                    if (!exists) {
                        ...
                        log.info("[subscription-service] Created database: {}", dbName);
                    } else {
                        log.info("[subscription-service] Database {} already exists", dbName);
                    }
                }
            }
        } catch (ClassNotFoundException e) {
            log.error("[subscription-service] PostgreSQL driver not found", e);
        } catch (SQLException e) {
            log.error("[subscription-service] Failed to check or create database", e);
        }
    }
}
